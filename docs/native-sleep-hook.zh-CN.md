...
若 root 私有状态文件 `/var/local/bjtu-dashboard/orientation` 的内容为 `right`，
基础图片已经为设备顺时针右转预旋转。此时钩子必须跳过按竖屏坐标绘制的实时
时间、日期和电量，只绘制基础 PNG 并执行最终 GC16 刷新。具体方向契约和界面
见[右转 90° 锁屏界面](rotation-layout.zh-CN.md)。
...
周期性后台更新的事件状态机、RTC 调度、联网窗口、令牌和失败回退规格见
[Kindle 休眠期间定时更新实现设计](scheduled-sleep-updates.zh-CN.md)。
...
- 右转模式必须由支持方向状态的 `render-panel.sh` 配合，未更新的旧钩子会把
  竖屏实时状态错误地叠加到预旋转图片上；

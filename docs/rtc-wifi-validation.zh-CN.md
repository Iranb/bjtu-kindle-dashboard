...
- 本地渲染及更新器测试共 10 项，全部通过；
- `raw.githubusercontent.com` 在设备当前网络下 12 秒内没有返回数据；
- 下载器按预期记录 `curl_28`，没有替换原面板；
- 同一仓库文件通过 GitHub Contents API 的 raw media type 返回 `200`，设备约
  1 秒取得 45,744 字节的合规 PNG，因此示例配置已改用该入口。

交接时仍未完成的实机项目：

- 用调整后的 Contents API 配置跑通下载器的 ETag、哈希和原子替换；
- 跑通一次完整的“RTC 唤醒—联网更新—自然重新挂起”循环；
- 将 USB 可见配置从 shell `source` 改成白名单解析器，消除不可信配置被 root
  当作代码执行的风险；
- 完成长周期循环、并发 RTC、用户抢占和耗电验收。

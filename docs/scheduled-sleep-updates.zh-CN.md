...
> 当前状态：仓库已经包含可独立安装的 `bjtu-dashboard-updater` 常驻服务。
> 它实现了 RTC 调度、`abortSuspend` 联网窗口、ETag、PNG 校验、原子替换、失败
> 退避和独立卸载。默认配置仍需完成连续循环、自然重新挂起和耗电验收后，才适合
> 作为面向其他设备的默认开启功能。
...
> 修正 RTC 写入时机和联网窗口流程。
>
> 当前实际部署的插电保持、5 分钟检查、45 秒 Wi-Fi 等参数及日常操作，以
> [Kindle 常驻屏保、联网与电源管理运维指南](kindle-power-operations.zh-CN.md)
> 和 `update.conf.example` 为准。本文后部的 10 分钟、12 秒数值保留为早期设计
> 记录，不再是当前默认值。
...
8. 计算 SHA-256，并与当前面板文件比较，内容相同则不刷新墨水屏；
